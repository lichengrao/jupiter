...
public class ItemRecommender {
    ...
    // Return a list of Item objects for the given type. Types are one of [Stream, Video, Clip]. Add items that are
    // related to the top games provided in the argument
    private List<Item> recommendByTopGame(ItemType type, List<Game> topGames) throws RecommendationException {
    ...
    // Return a list of Item objects for the given type. Types are one of [Stream, Video, Clip]. All items are
    // related to the items previously favorited by the user. E.g., if a user favorited some videos about game "Just
    // Chatting", then it will return some other videos about the same game.
    private List<Item> recommendByFavoriteHistory(Set<String> favoritedItemIds, List<String> favoriteGameIds,
        ...
        Map<String, Long> favoriteGameIdByCount =
                favoriteGameIds.parallelStream().collect(Collectors.groupingBy(Function.identity(),
                        Collectors.counting()));
    ...
    // Return a map of Item objects as the recommendation result. Keys of the map are [Stream, Video, Clip]. Each key
    // is corresponding to a list of Item objects, each item object is a recommended item based on the previous
    ...
    public Map<String, List<Item>> recommendItemsByUser(String userId) throws RecommendationException {
        ...
        for (Map.Entry<String, List<String>> entry : favoriteGameIds.entrySet()) {
            if (entry.getValue().size() == 0) {
                ...
                recommendedItemMap.put(entry.getKey(), recommendByTopGame(ItemType.valueOf(entry.getKey()), topGames));
            } else {
    ...
    // Return a map of Item objects as the recommendation result. Keys of the map are [Stream, Video, Clip]. Each key
    // is corresponding to a list of Items objects, each item object is a recommend item based on the top games
    // currently on Twitch.
    public Map<String, List<Item>> recommendItemsByDefault() throws RecommendationException {
        ...
        for (ItemType type : ItemType.values()) {
            recommendedItemMap.put(type.toString(), recommendByTopGame(type, topGames));
        }
        return recommendedItemMap;
    }
}
