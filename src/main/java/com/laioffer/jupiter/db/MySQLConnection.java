...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        // Need to make sure item is added to the database first because the foreign key restriction on item_id
        // (favorite_records) -> id(items)
        saveItem(item);
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                if (rs.next()) {
                    ...
                    Item item =
                            new Item.Builder().id(rs.getString("id")).title(rs.getString("title")).url(rs.getString(
                                    "url")).thumbnailUrl(rs.getString("thumbnail_url")).broadcasterName(rs.getString(
                                    "broadcaster_name")).gameId(rs.getString("game_id")).type(itemType).build();
    ...
    public boolean addUser(User user) throws MySQLException {
        ...
        try {
            ...
            e.printStackTrace();
            throw new MySQLException("Failed to add user to the Database");
        }
    }
}
